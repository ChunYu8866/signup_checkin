...
- 後端有近似速率限制（同一手機／E-mail 查詢每 10 分鐘 12 次、全站查詢每分鐘 240 次、現場報名全站每分鐘 20 筆），超量回 `BUSY` 由前端等候室退避。活動日若監控看到大量 `BUSY` 且現場人流不多，優先懷疑自動化濫用而非容量不足。
- Script Properties 修改即時生效，不需重新部署版本。
- 活動結束後請封存或停用 Web App 部署，端點不要無限期對外開放。
...
個資只存在一個地方：Google Sheet。防外洩的重點依序是——

1. **Sheet 共享設定（最重要）**：一般存取權必須是「限制」（僅限受邀者），不得使用「知道連結的任何人」。編輯者名單最小化，僅限實際需要的工作人員帳號；活動前逐一檢視「共用」名單並移除多餘帳號。
2. **擁有者帳號**：Sheet 與 Apps Script 擁有者的 Google 帳號必須開啟兩步驟驗證（2FA）。此帳號被盜等於名單與端點全部失守。
3. **正式名單匯入**：直接在 Google Sheet 介面貼上或匯入，處理完畢即刪除來源檔。名單檔案不得放進 repo 資料夾、不得 commit、不得經由聊天室或 E-mail 附件轉傳；單元測試會擋下被 commit 的試算表檔與大量手機號清單（`repo-hygiene.test.mjs`），但那是最後防線，不是授權管道。
4. **匯出與備份**：避免將含個資的匯出檔（xlsx/csv）散落在個人裝置；如需備份，存放於同等存取控制的雲端位置，活動後一併刪除。
5. **維運函式**：`initializeSheet`／`validateDeployment`／`warmIndexes`／`refreshIndexes` 僅能由擁有者在 Apps Script 編輯器執行；匿名網頁情境一律回 `OPERATOR_ONLY`（防匿名者取得名單筆數或濫用索引重建）。
6. **活動後收尾**：依核准的個資告知所載期間處理資料——逾期即刪除 Sheet 內個資（含垃圾桶清空）；封存或停用 Web App 部署，端點不再對外服務；`ALLOWED_ORIGINS` 等 Script Properties 可一併清空。
7. **疑似外洩應變**：立即將 Sheet 共享改為僅限擁有者、停用 Web App 部署，再清查 Apps Script 執行紀錄與 Sheet 版本歷史（檔案→版本記錄）判斷範圍，必要時依《個資法》第 12 條通知當事人並知會法遵。
